...
仓库中的 [`browser-extension/`](./browser-extension/) 是无需构建的 Chrome/Edge Manifest V3 扩展。打开浏览器扩展管理页、启用开发者模式并选择“加载已解压的扩展程序”，然后在“自选 → 设置 → 浏览器助手”生成配对码。扩展使用用户主动点击获得的当前标签页权限，可以把网页加入稍后看或合集、识别并订阅当前来源，也能在 B站关注页中逐页执行“扫描本页”后批量导入。它不申请 Cookie、历史记录或全站常驻读取权限。
