...
- B站使用站内链接模式。如有自建 RSSHub 等转换地址，可作为普通 RSS 添加。
- 外部页面优先在受限 iframe 中加载。若来源通过 `X-Frame-Options` 或 CSP 禁止内嵌，界面会保留“在当前页打开”的降级入口。
- 来源平台的登录状态由浏览器 Cookie 策略管理。B站等平台若在 iframe 中登录后仍显示未登录，请使用查看器中的“在当前页打开并登录”进入第一方登录环境，再通过浏览器返回。JSON 备份不会包含密码或 Cookie，只会包含非敏感的平台地址与最近打开时间。
